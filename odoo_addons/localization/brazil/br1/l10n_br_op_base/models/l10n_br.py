...
class L10nBR(models.AbstractModel):
    ...
    l10n_br_localization = fields.Boolean(compute="_compute_is_br_localization", string='Is BR localization?')
    ...
    def _get_user_localization(self):
        ...
        br_template = self.env['ir.model.data'].get_object('br_coa_simple', 'br_account_chart_template')
        br_template_id = br_template and br_template.id
        return br_template_id == template_id

    @api.model
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ret_val = super(L10nBR, self).fields_view_get( view_id=view_id, view_type=view_type,
        ...
        for field in ret_val['fields']:
            ...
            for node in doc.xpath("//field[@name='%s']" % field):
                mod_field = (view_type == 'tree' and 'column_invisible' or 'invisible')
                modifiers = json.loads(node.get("modifiers"))
                if view_type == 'tree':
                    modifiers[mod_field] = not self._get_user_localization()
                if view_type == 'form':
                    domain = modifiers.get(mod_field, [])
                    domain = expression.OR([domain, [('l10n_br_localization', '=', False)]])
        ...
        for record in self:
            if hasattr(record, 'company_id'):
                user_template_id = (record.company_id.chart_template_id.id
                                    or user_template_id)
            br_template = record.env['ir.model.data'].get_object('br_coa_simple', 'br_account_chart_template')
            br_template_id = br_template and br_template.id
