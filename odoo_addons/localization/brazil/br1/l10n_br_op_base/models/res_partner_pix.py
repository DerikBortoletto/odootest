...
class PartnerPix(models.Model):
    ...
    partner_bank_id = fields.Many2one(comodel_name="res.partner.bank", string="Bank Account", domain="[('partner_id', '=', partner_id)]")
    ...
    def _normalize_cnpj_cpf(self, doc_number):
        ...
        if len(doc_number) == 11 and not fiscal.validate_cnpj(doc_number):
            raise ValidationError(_('Invalid CNPJ Number!'))
        elif len(doc_number) == 14 and not fiscal.validate_cpf(doc_number):
