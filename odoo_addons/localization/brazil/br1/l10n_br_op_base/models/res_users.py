...
class ResUsers(models.Model):
    ...
    def notify_info(self, message, title=None, sticky=False, redirect=False):
        ...
        self._notify_channel('notify_info_channel_name', message, title, sticky, redirect)

    def notify_warning(self, message, title=None, sticky=False,
                       redirect=False):
        title = title or _('Warning')
        self._notify_channel('notify_warning_channel_name', message, title, sticky, redirect)

    def _notify_channel(self, channel_name_field, message, title, sticky, redirect):
        bus_message = {
            'message': message,
            'title': title,
            'sticky': sticky,
        ...
        notifications = [(getattr(record, channel_name_field), bus_message) for record in self]
        self.env['bus.bus'].sendmany(notifications)

    def notify(self, message, title=None, sticky=None, redirect=False, warning=False):
        if warning:
