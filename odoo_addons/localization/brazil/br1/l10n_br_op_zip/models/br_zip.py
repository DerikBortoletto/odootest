...
class BrZip(models.Model):
    ...
    city_id = fields.Many2one('res.state.city', 'Cidade', required=True, domain="[('state_id','=',state_id)]")

    def set_domain(self, country_id=False, state_id=False, city_id=False, district=False, street=False, zip_code=False):
        domain = []
        if zip_code:
    ...
    def zip_search_multi(self, country_id=False, state_id=False, city_id=False,
                         district=False, street=False, zip_code=False):
        domain = self.set_domain(country_id=country_id, state_id=state_id, city_id=city_id, 
    ...
    def _search_by_cep(self, zip_code):
        try:
            ...
            state = self.env['res.country.state'].search([('country_id.code', '=', 'BR'),('code', '=', res['uf'])])
            city = self.env['res.state.city'].search([('name', '=ilike', res['cidade']),('state_id', '=', state.id)])
    ...
    def _search_by_address(self, city_id, street):
        try:
            ...
            url_viacep = 'http://viacep.com.br/ws/' + city.state_id.code + '/' + city.name + '/' + street + '/json/'
            obj_viacep = requests.get(url_viacep)
            results = obj_viacep.json()
            if results:
                for res in results:
                    city = self.env['res.state.city'].search([('ibge_code', '=', res['ibge'][2:]),
    ...
    def search_by_address(self, obj, country_id=False, state_id=False, city_id=False, 
                          district=False, street=False, error=True):
        zip_ids = self.zip_search_multi(country_id=country_id, state_id=state_id, city_id=city_id,
        ...
        if len(zip_ids) == 1:
            ...
            zip_ids = obj_zip_result.map_to_zip_result(zip_ids, obj._name, obj.id)

            return self.create_wizard(obj._name, obj.id, country_id=obj.country_id.id, state_id=obj.state_id.id,
                                      city_id=obj.city_id.id, district=obj.district, street=obj.street, zip_code=obj.zip,
    ...
    def create_wizard(self, object_name, address_id, country_id=False, state_id=False, city_id=False, 
                      district=False, street=False, zip_code=False, zip_ids=False):
