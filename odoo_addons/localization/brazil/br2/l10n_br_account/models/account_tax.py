...
class AccountTax(models.Model):
    ...
    def compute_all(
        ...
        taxes_results["amount_tax_included"] = 0.0 # fiscal_taxes_results["amount_included"]
        taxes_results["amount_tax_not_included"] = 0.0 # fiscal_taxes_results["amount_not_included"]
        ...
        for account_tax in taxes_results["taxes"]:
            ...
            fiscal_tax = fiscal_taxes_results["taxes"].get(account_taxes_by_domain.get(tax.id))
            ...
            tax_repartition_lines = (is_refund and tax.refund_repartition_line_ids or \
                                     tax.invoice_repartition_line_ids).filtered(lambda x: x.repartition_type == "tax")

            sum_repartition_factor = sum(tax_repartition_lines.mapped("factor"))

            if fiscal_tax:
                if fiscal_tax.get('tax_include',False):
                    taxes_results["amount_tax_included"] += fiscal_tax['tax_value']
                else:
                    taxes_results["amount_tax_not_included"] += fiscal_tax['tax_value']
