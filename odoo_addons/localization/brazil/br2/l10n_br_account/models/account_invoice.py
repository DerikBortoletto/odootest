...
class AccountMove(models.Model):
    ...
    def fields_view_get(self, view_id=None, view_type="form", toolbar=False, submenu=False):
        ...
        if view_type == "form":
            ...
            if view_id == self.env.ref("l10n_br_account.fiscal_invoice_form").id:
                invoice_line_form_id = self.env.ref("l10n_br_account.fiscal_invoice_line_form").id
                sub_form_view = self.env["account.move.line"].fields_view_get(view_id=invoice_line_form_id, view_type="form")["arch"]
                sub_form_node = self.env["account.move.line"].inject_fiscal_fields(sub_form_view)
                sub_arch, sub_fields = view.postprocess_and_fields(sub_form_node, "account.move.line", False)
                ...
                if invoice_view["fields"].get("invoice_line_ids"):
                    invoice_line_form_id = self.env.ref("l10n_br_account.invoice_form").id
                    sub_form_view = invoice_view["fields"]["invoice_line_ids"]["views"]["form"]["arch"]
                ...
                if invoice_view["fields"].get("line_ids"):
                    invoice_line_form_id = self.env.ref("l10n_br_account.invoice_form").id
                    sub_form_view = invoice_view["fields"]["line_ids"]["views"]["tree"]["arch"]
    ...
    def write(self, values):
        # if bool(values.get('invoice_line_ids',False)) and not bool(values.get('line_ids',False)):
        #     values['line_ids'] = values.pop('invoice_line_ids', None)
    ...
    def unlink(self):
        ...
        for move in self:
            if not move.exists():
                continue
            if (move.fiscal_document_id and move.fiscal_document_id.id != self.env.company.fiscal_dummy_id.id):
        ...
        for line in self.line_ids.filtered(lambda line: not line.tax_repartition_line_id):
            ...
            for tax_vals in compute_all_vals["taxes"]:
                ...
                if taxes_mapped[grouping_key]:
                    ...
                    taxes_mapped[grouping_key]["tax_base_amount"] += self._get_base_amount_to_display(tax_vals["base"], tax_repartition_line, tax_vals["group"])
    ...
    def _recompute_payment_terms_lines(self):
        ...
        if self.document_number:
            terms_lines = self.line_ids.filtered(lambda l: l.account_id.user_type_id.type in ("receivable", "payable") and l.move_id.document_type_id)
            ...
            for idx, terms_line in enumerate(terms_lines):
                ...
                terms_line.name = "{}/{}-{}".format(self.document_number, idx + 1, len(terms_lines))
        return result
    ...
    def action_post(self):
       ...
       # Hack: Na V14 o frete, seguro e outros custos não estão no lancamento contabil
        ...
        self.mapped("fiscal_document_id").filtered(lambda d: d.document_type_id).action_document_confirm()
    ...
    def view_pdf(self):
        self.ensure_one()
        try:
            file_pdf = self.fiscal_document_id.view_pdf()
