...
class AccountMoveLine(models.Model):
    ...
    lot_ids = fields.One2many(related="fiscal_document_line_id.lot_ids", string="Traceability")
    ...
    def _prepare_shadowed_fields_dict(self, default=False):
        ...
        lot_ids = self._prepare_lots_dict()
        if bool(lot_ids):
            vals['lot_ids'] = lot_ids
        if default:  # in case you want to use new rather than write later
            return {"default_%s" % (k,): vals[k] for k in vals.keys()}
        return vals

    def _prepare_lots_dict(self):
        """ Prepara os campos dos lotes vindo dos pickings para adcionar no eDoc """
        res = []
        if hasattr(self, 'prod_lot_ids'):
            res += [(5,)]
            for move_stock in self.move_line_ids:
                for lot in move_stock.lot_ids:
                    var = {
                        'document_line_id': self.id,
                        'name': lot.name,
                        'product_qty': move_stock.product_qty,
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            fiscal_doc_id = (self.env["account.move"].browse(values["move_id"]).fiscal_document_id.id)
            if fiscal_doc_id == dummy_doc.id or values.get("exclude_from_invoice_tab"):
                if len(dummy_line) < 1:
                    raise UserError(_("Document line dummy not found. Please contact your system administrator."))
