...
def filter_processador_edoc_nfe(record):
    if record.processador_edoc in [PROCESSADOR_OCA,PROCESSADOR_TRUST] and \
       record.document_type_id.code in [MODELO_FISCAL_NFE, MODELO_FISCAL_NFCE]:
        return True
    return False
...
> <infnfe>
    ...
    nfe_version = fields.Selection(selection=NFE_VERSIONS, string="NFe Version", copy=False, default=lambda self: self.env.company.nfe_version,)

    nfe40_Id = fields.Char(compute="_compute_id_tag", inverse="_inverse_nfe40_Id",)
    ...
    def _compute_id_tag(self):
        ...
        for record in self.filtered(filter_processador_edoc_nfe):
            ...
            if (
                ...
                record.nfe40_Id = "{}{}".format(record.document_type_id.prefix, record.document_key)
            else:
    ...
    nfe40_cUF = fields.Char(related="company_id.partner_id.state_id.ibge_code", string="nfe40_cUF",)
    ...
    nfe40_tpNF = fields.Selection(compute="_compute_ide_data", inverse="_inverse_nfe40_tpNF",)
    ...
    nfe40_tpImp = fields.Selection(compute="_compute_ide_data", inverse="_inverse_nfe40_tpImp",)

    danfe_layout = fields.Selection(selection=NFE_DANFE_LAYOUTS + NFCE_DANFE_LAYOUTS,)

    nfe40_tpEmis = fields.Selection(related="nfe_transmission")  # TODO no caso de entrada

    nfe_transmission = fields.Selection(selection=NFE_TRANSMISSIONS, string="NFe Transmission", copy=False,
    ...
    nfe_environment = fields.Selection(selection=NFE_ENVIRONMENTS, string="NFe Environment", copy=False,
    ...
    nfe40_verProc = fields.Char(copy=False, default=lambda s: s.env["ir.config_parameter"].sudo()
                                .get_param("l10n_br_nfe.version.name", default="TrackErp 14.0"),)
    ...
    nfe40_NFref = fields.One2many(comodel_name="l10n_br_fiscal.document.related", related="document_related_ids", 
    ...
    nfe40_emit = fields.Many2one(comodel_name="res.company", compute="_compute_emit_data", readonly=True, string="Emit",)

    nfe40_CRT = fields.Selection(related="company_tax_framework", string="Código de Regime Tributário (NFe)",)
    ...
    nfe40_dest = fields.Many2one(comodel_name="res.partner", compute="_compute_dest_data", readonly=True,
                                 string="Dest",)

    nfe40_indIEDest = fields.Selection(related="partner_ind_ie_dest", string="Contribuinte do ICMS (NFe)")
    ...
    nfe40_det = fields.One2many(comodel_name="l10n_br_fiscal.document.line", inverse_name="document_id", related="fiscal_line_ids",)
    ...
    nfe40_vDesc = fields.Monetary(string="Montante de Desconto", related="amount_discount_value")
    ...
    nfe40_vPIS = fields.Monetary(string="Valor do PIS (NFe)", related="amount_pis_value")

    nfe40_vCOFINS = fields.Monetary(string="valor do COFINS (NFe)", related="amount_cofins_value")

    nfe40_vOutro = fields.Monetary(string="Outros Custos", related="amount_other_value")
    ...
    nfe40_infRespTec = fields.Many2one(comodel_name="res.partner", related="company_id.technical_support_id")
    ...
    nfe40_autXML = fields.One2many(comodel_name="res.partner", default=_default_nfe40_autxml)
    ...
    def _serialize(self, edocs):
        ...
        for record in self.with_context(lang="pt_BR").filtered(filter_processador_edoc_nfe):
    ...
    def _document_export(self, pretty_print=True):
        ...
        for record in self.filtered(filter_processador_edoc_nfe):
            ...
            xml_file = processador._generateds_to_string_etree(edoc, pretty_print=pretty_print)[0]
            _logger.debug(xml_file)
            event_id = self.event_ids.create_event_save_xml(
                company_id=self.company_id,
                environment=(EVENT_ENV_PROD if self.nfe_environment == "1" else EVENT_ENV_HML),
                event_type="0",
                xml_file=xml_file,
                document_id=self)
            ...
            elif record.processador_edoc == PROCESSADOR_TRUST:
                from pytrustnfe.certificado import Certificado
                from pytrustnfe.nfe.assinatura import Assinatura

                parser = etree.XMLParser(remove_blank_text=True, remove_comments=True, strip_cdata=False)
                xml_tree = etree.fromstring(xml_file, parser=parser)
                
                Cert = record.company_id.certificate_nfe_id.file
                if Cert:
                    Cert_pfx = base64.decodebytes(Cert)
                    certificado = Certificado(Cert_pfx, record.company_id.certificate_nfe_id.password)
                    signer = Assinatura(certificado.pfx, certificado.password)
                    xml_assinado = signer.assina_xml(xml_tree, edoc.infNFe.Id)
                    self._valida_xml(xml_assinado)
        return result
        ...
        for record in self.filtered(filter_processador_edoc_nfe):
            ...
            if record.processador_edoc == PROCESSADOR_OCA:
                ...
                for edoc in record.serialize():
                    ...
                    for p in processador.processar_documento(edoc):
                        ...
                        if processo.webservice == "nfeAutorizacaoLote":
                            record.authorization_event_id._save_event_file(processo.envio_xml.decode("utf-8"), "xml")
    
                if processo.resposta.cStat in LOTE_PROCESSADO + ["100"]:
                    record.atualiza_status_nfe(processo.protocolo.infProt, processo.processo_xml.decode("utf-8"))
                ...
                for edoc in record.serialize():
                    ...
                    xml_file = base64.decodebytes(record.send_file_id.datas).decode('utf-8')
                    idLote = datetime.now().strftime('%Y%m%d%H%M%S')
                    indSinc = 1 if record.company_id.nfe_sinc else 0
                    lote = """<enviNFe xmlns="http://www.portalfiscal.inf.br/nfe" versao="4.00">
                                <idLote>{idLote}</idLote>
                                <indSinc>{indSinc}</indSinc>
                                {infNFe}
                              </enviNFe>""".format(idLote=idLote,indSinc=indSinc,infNFe=xml_file)

                    parser = etree.XMLParser(remove_blank_text=True, remove_comments=True, strip_cdata=False)
                    xml_tree = etree.fromstring(lote, parser=parser)
                    ...
                    if Cert:
                        ...
                        certificado = Certificado(Cert_pfx, record.company_id.certificate_nfe_id.password)
                        signer = Assinatura(certificado.pfx, certificado.password)
                        xml_to_send = signer.assina_xml(xml_tree, edoc.infNFe.Id)
    ...
    def view_pdf(self):
        ...
        # if self.processador_edoc in [PROCESSADOR_OCA]:
        #     if not self.authorization_file_id or not self.file_report_id:
        #         self.make_pdf()
        #     return self._target_new_tab(self.file_report_id)
        # elif self.processador_edoc in [PROCESSADOR_TRUST]:
        danfe_name = "l10n_br_fiscal.main_template_br_nfe_danfe" 
        danfe_report = self.env['ir.actions.report'].search([('report_name', '=', danfe_name)])
        report_service = danfe_report.xml_id
        danfe, dummy = self.env.ref(report_service)._render_qweb_pdf([self.id])
    ...
    def make_pdf(self):
        ...
        if self.correction_event_ids:
            xml_string_cce = ''
            for event in self.correction_event_ids:
                arquivo_cce = event.file_request_id
                xml_string_cce += base64.b64decode(arquivo_cce.datas).decode()
        else:
            xml_string_cce = None
                

        pdf = base.ImprimirXml.imprimir(string_xml=xml_string,cce_xml=xml_string_cce) # output_dir=self.authorization_event_id.file_path
