...
class FiscalDocumentMixin(models.AbstractModel):
    ...
    is_importa = fields.Boolean(string = "É importação ? ")

    looping_customer = fields.Char(compute = "_computed"
    ...
    def create(self, vals):
        self._looping(vals)
        return super(FiscalDocumentMixin, self).create(vals)

    def write(self, vals):
        self._looping(vals)
        res = super(FiscalDocumentMixin, self).write(vals)
        return res


    def _looping(self,vals):
        nome = str(self)
        valid = nome.find('purchase')
        if valid != -1:
            for i in self.order_line:
                lista = []
                for f in self.nDI_value:
                    lista.append(f)
                if len(lista) > 0:
                    i.write({"nDI_value": lista[0]
                    })
