...
class Comment(models.Model):
    ...
    comment_type = fields.Selection(selection=COMMENT_TYPE, default=COMMENT_TYPE_COMMERCIAL, required=True)

    object = fields.Selection(selection=FISCAL_COMMENT_OBJECTS, required=True)

    date_begin = fields.Date(string="Initial Date")

    date_end = fields.Date(string="Final Date")

    object_id = fields.Reference(string="Reference", selection=FISCAL_COMMENT_OBJECTS, ondelete="set null")
    ...
    def _name_search(self, name, args=None, operator="ilike", limit=100, name_get_uid=None):
        ...
        domain = []
        if name:
            domain = ["|",("name", operator, name),("comment", "ilike", "%" + name + "%"),]
    ...
    def compute_message(self, vals, manual_comment=None):
        ...
        mako_template_env = SandboxedEnvironment(block_start_string="<%",block_end_string="%>",
                                                 variable_start_string="${",variable_end_string="}",comment_start_string="<%doc>",
                                                 comment_end_string="</%doc>", line_statement_prefix="%", line_comment_prefix="##",
        ...
        mako_template_env.globals.update(
            {
                ...
                "format_amount": (lambda amount, context=self._context: self.format_amount(self.env, amount, self.env.ref("base.BRL"))),
            }
        )
