...
class FiscalDocumentLineMixinMethods(models.AbstractModel):
    ...
    @api.model
    def inject_fiscal_fields(self, view_arch, view_ref="l10n_br_fiscal.document_fiscal_line_mixin_form", xpath_mappings=None,):
        """
        ...
        if xpath_mappings is None:
            xpath_mappings = (
                ...
                (".//page[@name='fiscal_line_extra_info']","//page[@name='fiscal_line_extra_info']",),
                # these will only collect (invisible) fields for onchanges:
                (".//control[@name='fiscal_taxes_fields']...","//page[@name='fiscal_taxes']//field",),
                (".//control[@name='fiscal_line_extra_info_fields']...","//page[@name='fiscal_line_extra_info']//field",),
            )
    ...
    @api.depends("fiscal_price","discount_value","insurance_value","other_value","freight_value","fiscal_quantity",
                 "amount_tax_not_included","uot_id","product_id","partner_id","company_id")
    def _compute_amounts(self):
        for record in self:
            ...
            record.amount_fiscal = (round_curr.round(record.fiscal_price * record.fiscal_quantity)
            ...
            record.amount_total = (record.amount_untaxed + record.amount_tax + add_to_amount - rm_to_amount)
