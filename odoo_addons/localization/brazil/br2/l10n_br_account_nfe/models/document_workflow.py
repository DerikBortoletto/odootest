...
def filter_nfe(record):
    if (
        record.processador_edoc in [PROCESSADOR_OCA,PROCESSADOR_TRUST] and record.document_type_id.code
        in [
            MODELO_FISCAL_NFE,
            MODELO_FISCAL_NFCE,
        ]
            ...
            if record.amount_financial_total and record.edoc_purpose not in (
                ...
                if record.fiscal_operation_type == 'out':
                    for mov in record.move_ids.financial_move_line_ids:
                        ...
                        count += 1
                        # [WIP] Check validation for nfe40_dup.display_name error when in UI user click at duplicate record to see it
                        # if record.nfe40_dup:
                        #     record.nfe40_dup.display_name = record.display_name
                if record.fiscal_operation_type == 'in':
                    for mov in record.move_ids.financial_move_line_ids:
                        ...
                        # if record.nfe40_dup:
                        #     record.nfe40_dup.display_name = record.display_name
                record.nfe40_dup = [(6, 0, duplicatas.id)]
