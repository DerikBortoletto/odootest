...
class AbstractSpecMixin(models.AbstractModel):
    ...
    def _build_generateds(self, class_name=False):
        ...
        for i in class_obj._fields:
            if class_obj._fields[i].name.startswith(class_obj._field_prefix) and "_choice" not in class_obj._fields[i].name:
                tst_field.append(class_obj._fields[i].name)
        print(tst_field)
        
        xsd_fields = (i for i in class_obj._fields if class_obj._fields[i].name.startswith(class_obj._field_prefix)
