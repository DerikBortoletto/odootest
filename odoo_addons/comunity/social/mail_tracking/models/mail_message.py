...
class MailMessage(models.Model):
    ...
    email_cc = fields.Char("Cc", 
                           help="Additional recipients that receive a " '"Carbon Copy" of the e-mail')
    email_to = fields.Char("To", help="Raw TO recipients")
    mail_tracking_ids = fields.One2many(comodel_name="mail.tracking.email", inverse_name="mail_message_id",
                                        string="Mail Trackings")
    mail_tracking_needs_action = fields.Boolean(default=False, help="The message tracking will "
                                                "be considered" " to filter tracking issues")
    ...
    def _compute_is_failed_message(self):
        ...
        for message in self:
            ...
            involves_me = self.env.user.partner_id in (message.author_id | message.notification_ids.mapped("res_partner_id"))
            has_failed_trackings = failed_states.intersection(message.mapped("mail_tracking_ids.state"))
            message.is_failed_message = bool(needs_action and involves_me and has_failed_trackings)
    ...
    def _search_is_failed_message(self, operator, value):
        ...
        notification_partner_ids = self.search([("notification_ids.res_partner_id", "=", self.env.user.partner_id.id)])
    ...
    def tracking_status(self):
        ...
        for message in self:
            ...
            trackings = (self.env["mail.tracking.email"].sudo().search([("mail_message_id", "=", message.id)]))
            # String to List
