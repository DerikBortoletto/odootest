...
class AccountStatementImport(models.TransientModel):
    ...
    def _import_file(self):
        ...
        if not result["statement_ids"]:
            raise UserError(_("You have already imported this file, or this file "
    ...
    def import_single_file(self, file_data, result):
        ...
        logger.info("Bank statement file %s contains %d accounts", self.statement_filename, len(parsing_data))
        i = 0
    ...
    def import_single_statement(self, single_statement_data, result):
        if not isinstance(single_statement_data, tuple):
            raise UserError(_("The parsing of the statement file returned an invalid result."))
        ...
        if not journal.default_account_id:
            raise UserError(_("The Bank Accounting Account in not set on the " "journal '%s'.") % journal.display_name)
    ...
    def _parse_file(self, data_file):
        ...
        a list of triplets.
        """
        raise UserError(_("This bank statement file format is not supported.\n"
    ...
    def _match_currency(self, currency_code):
        ...
        if not currency:
            raise UserError(_("The bank statement file uses currency '%s' "
    ...
    def _match_journal(self, account_number, currency):
        ...
        if not account_number:  # exemple : QIF
            if not self.env.context.get("journal_id"):
                raise UserError(_("The format of this bank statement file doesn't "
            ...
            journal = journal_obj.search([("type", "=", "bank"),
                                          ("bank_account_id.sanitized_acc_number","ilike",sanitized_account_number,)],limit=1)

            if not journal:
                bank_accounts = self.env["res.partner.bank"].search([("partner_id", "=", company.partner_id.id),
                                                                     ("sanitized_acc_number", "ilike", sanitized_account_number)],
                                                                     limit=1)
                if bank_accounts:
                    raise UserError(_("The bank account with number '%s' exists in Odoo "
                    ...
                    raise UserError(_("Could not find any bank account with number '%s' "
                                      ...
                                      "should create a new one.") % (account_number, company.partner_id.display_name))
        ...
        if journal_currency != currency:
            raise UserError(_("The currency of the bank statement (%s) is not the same as the "
                              "currency of the journal '%s' (%s).") % (currency.name, journal.display_name, journal_currency.name))
        return journal
    ...
    def _complete_stmts_vals(self, stmts_vals, journal, account_number):
        ...
        for st_vals in stmts_vals:
            ...
            for lvals in st_vals["transactions"]:
                journal._statement_line_import_update_unique_import_id(lvals, account_number)
        ...
        for st_vals in stmts_vals:
            ...
            for lvals in st_vals["transactions"]:
                ...
                if lvals.get("unique_import_id"):
                    existing_line = absl_obj.sudo().search([("unique_import_id", "=", lvals["unique_import_id"])], limit=1)
        ...
        if num_ignored > 0:
            result["notifications"].append({
                ...
                "message": _("%d transactions had already been imported and were ignored.") % num_ignored 
                           if num_ignored > 1 else _("1 transaction had already been imported and was ignored."),
                "details": {
