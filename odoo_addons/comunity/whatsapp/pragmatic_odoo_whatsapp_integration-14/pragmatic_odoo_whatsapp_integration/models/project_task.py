...
class ProjectTask(models.Model):
    ...
    def send_message_on_whatsapp(self, whatsapp_instance_id):
        if self.user_id.partner_id.mobile:
            if self.user_id.partner_id.country_id.phone_code and self.user_id.partner_id.mobile:
                msg = ''
                mail_message_body = ''
                whatsapp_msg_number = self.user_id.partner_id.mobile
                whatsapp_msg_number_without_space = whatsapp_msg_number.replace(" ", "")
                whatsapp_msg_number_without_code = whatsapp_msg_number_without_space.replace('+' + str(self.user_id.partner_id.country_id.phone_code), "")
                ...
                msg = _("Hello") + " " + self.user_id.partner_id.name + "," + "\n" + _("New task assigned to you") + "\n" + msg
                ...
                mobile_with_country = str(self.user_id.partner_id.country_id.phone_code) + "" + whatsapp_msg_number_without_code
                tmp_dict = {
                ...
                if response.status_code == 201 or response.status_code == 200:
                    ...
                    if not json_send_message_response.get('sent') and json_send_message_response.get('error') and json_send_message_response.get(
                        ...
                        raise UserError(_('Please add valid whatsapp number for %s ') % self.user_id.partner_id.name)
    ...
    def send_message_from_gupshup_without_template(self, whatsapp_instance_id):
        ...
        msg = ''
        mail_message_body = ''
        if self.user_id.partner_id.mobile and self.user_id.partner_id.country_id.phone_code and self.user_id.partner_id.mobile:
            whatsapp_number = self.user_id.partner_id.mobile
        ...
        if self.user_id.partner_id.mobile:
            if self.user_id.partner_id.country_id.phone_code and self.user_id.partner_id.mobile:
                ...
                if '+' in whatsapp_msg_number_without_space:
                    number_without_code = whatsapp_msg_number_without_space.replace('+' + str(self.user_id.partner_id.country_id.phone_code), "")
                else:
                    number_without_code = whatsapp_msg_number_without_space.replace(str(self.user_id.partner_id.country_id.phone_code), "")
                response = None
                if not whatsapp_instance_id.project_task_add_signature:
                    response = self.gupshup_template_project_task_without_signature(url, headers, self.user_id, number_without_code, whatsapp_instance_id)
                    message = self.create_project_task_message(self.user_id.partner_id, whatsapp_instance_id, signature=False)

                else:
                    response = self.gupshup_template_project_task_with_signature(url, headers, self.user_id, number_without_code, whatsapp_instance_id)
                    message = self.create_project_task_message(self.user_id.partner_id, whatsapp_instance_id, signature=True)
                if response.status_code == 201 or response.status_code == 200 or response.status_code == 202:
                    json_response = json.loads(response.text)
                    _logger.info("\nSend Message successfully")
                    mobile_with_country = str(self.user_id.partner_id.country_id.phone_code) + number_without_code
                    self.env['whatsapp.msg.res.partner'].with_context({'partner_id': self.user_id.partner_id.id}).gupshup_create_whatsapp_message(
                        str(self.user_id.partner_id.country_id.phone_code) + number_without_code, message, json_response.get('messageId'), whatsapp_instance_id, 'project.task',
                        self)
    ...
    def send_message_on_whatsapp_through_template(self, whatsapp_instance_id):
        ...
        if self.user_id.partner_id.mobile:
            if self.user_id.partner_id.country_id.phone_code and self.user_id.partner_id.mobile:
                ...
                response = None
                if not whatsapp_instance_id.project_task_add_signature:
                    response = self.chat_api_template_project_task_without_signature(url, headers, self.user_id, number_without_code, whatsapp_instance_id)
                    message = self.create_project_task_message(self.user_id.partner_id, whatsapp_instance_id, signature=False)

                else:
                    response = self.chat_api_template_project_task_with_signature(url, headers, self.user_id, number_without_code, whatsapp_instance_id)
                    message = self.create_project_task_message(self.user_id.partner_id, whatsapp_instance_id, signature=True)
                if response.status_code == 201 or response.status_code == 200:
                    json_response = json.loads(response.text)
                    if json_response.get('sent') and json_response.get('description') == 'Message has been sent to the provider':
                        ...
                        raise UserError(_('Please add valid whatsapp number for %s customer') % self.user_id.partner_id.name)
