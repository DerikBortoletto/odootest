...
class Products(http.Controller):
    ...
    def get_product_name_id(self, *, p_name=None, p_code=None, p_id=None, raise_exception=True):
        ...
        if bool(p_name):
            pid = None
            data = self.request.env["product.product"].sudo().search([("company_id", "=", int(self.company_id)), ("name", "=", p_name)])
            if bool(data):
                for each in data:
                    pid = each.id
            _res = pid
        ...
        if bool(p_code):
            pid = None
            data = self.request.env["product.product"].sudo().search([("company_id", "=", int(self.company_id)), ("barcode", "=", p_code)])
            if bool(data):
                for each in data:
                    pid = each.id
            _res = pid
        ...
        if bool(p_id):
            p_name = None
            data = self.request.env["product.product"].sudo().search([("company_id", "=", int(self.company_id)), ("id", "=", int(p_id))])
            if bool(data):
                for each in data:
                    p_name = each.name
            _res = p_name
    ...
    def get_active_products_list(self, inputs):
        ...
        if "product_name" in inputs and bool(inputs["product_name"]):
            ...
            _query += """ AND LOWER(PT.name) LIKE '%s'"""
    ...
    def product_creation(self, inputs):
            ...
            for each in inputs["attributes"]:
                ...
                for key, val in each.items():
                    ...
                    if bool(attribute_exists):
                        ...
                        for _v in val:
                            attribute_value_exists = self.request.env["product.attribute.value"].sudo().search([("attribute_id", "=", attribute_id), ("name", "ilike", _v)])
        ...
        if is_ready:
            ...
            if bool(product_created):
                product = self.request.env["product.product"].sudo().search([("product_tmpl_id", "=", int(product_created[-1].id))])
                update_product_expiry = product.sudo().write({"use_expiration_date": True if payload["tracking"] != "none" else False})
    ...
    def check_if_product_exists(self, inputs):
        ...
        if bool(product):
            product.sudo().write({"tracking": inputs["product_tracking"] if inputs["product_tracking"] != "none" else "none", "use_expiration_date":  True if inputs["product_tracking"] != "none" else False})
            res["product_id"] = product[-1].id
