...
class Inbounds(http.Controller):
        ...
        if bool(po):
            ...
            if bool(line_item_ids):
                ...
                for li_id in line_item_ids:
                    ...
                    products_qty_to_receive.append({"product_id": str(product[0].id), "qty_to_receive": line_item[-1].product_qty - line_item[-1].qty_received})
                                ...
                                if go:
                                    if "product_id" in _l_i and int(_l_i["product_id"]) in products_id:
                                        ...
                                        if bool(_check_ls_name):
                                            lot_serial = self.request.env["stock.production.lot"].sudo().search([("name", "=", _check_ls_name)])
                                            if bool(lot_serial):
                                        ...
                                        for each in products_qty_to_receive:
                                            if int(each["product_id"]) == int(line_item_product_id):
                                                if int(each["qty_to_receive"]) > len(serials):
                                                    is_backorder = True
                                                if int(each["qty_to_receive"]) < len(serials):
                                                    raise Exception("Receiving quantity of a serial-based product is not possible to process!")
                                        ...
                                        for each in products_qty_to_receive:
                                            if int(each["product_id"]) == int(line_item_product_id):
                                                if float(each["qty_to_receive"]) > total_done_qty:
                                                    is_backorder = True
                                                if float(each["qty_to_receive"]) < total_done_qty:
                                                    raise Exception("Receiving quantity of a lot-based product is not possible to process!")
                                        ...
                                        for each in products_qty_to_receive:
                                            if int(each["product_id"]) == int(line_item_product_id):
                                                if float(each["qty_to_receive"]) > float(_l_i["qty_done"]):
                                                    is_backorder = True
                                                if float(each["qty_to_receive"]) < float(_l_i["qty_done"]):
                                                    raise Exception("Receiving quantity of a product is not possible to process!")
                                        if go:
                                            temp = dict()
                                            temp["product_uom_id"] = int(product_uom_id)
                                            temp["product_id"] = int(_l_i["product_id"])
                            ...
                            if bool(stock_move):
                                ...
                                if qty_to_pick > 0:
                                    if float(stock_move[-1].product_qty) - float(stock_move[-1].quantity_done) >= qty_to_pick:
                                        stock_move.sudo().write({"move_line_nosuggest_ids": [(0, False, _line) for _line in _line_items_to_move]})
                                        go = True
                        if go:
                            try:
                                stock_picking.button_validate()
                    ...
                    for _each in stock_pickings:
                        if _each.state == "done":
                            ...
                            for i in range(len(move_line_ids)):
                                stock_move_line = self.request.env["stock.move.line"].sudo().search([("id", "=", move_line_ids[i])])
                                if bool(stock_move_line):
                                    if int(stock_move_line[-1].product_id[0].id) == int(p_id) and float(stock_move_line[-1].qty_done) > 0:
                                        if not bool(_tracking):
                                            if bool(stock_move_line[-1].lot_id):
                                                if float(stock_move_line[-1].qty_done) == 1:
                                                    _tracking = "serial"
                                                else:
                                                    _tracking = "lot"
                                            else:
                ...
                if bool(_last_received_as):
                    if _last_received_as == "lot":
                        if bool(inputs["serial"]):
                            ...
                            _res["msg"] = "The product code " + str(inputs["product_code"]) + " was received as lot-based last time, so serials not allowed!"
                        elif not bool(inputs["lot_number"]):
                            exception_found = True
                            _res["msg"] = "The product code " + str(inputs["product_code"]) + " was received as lot-based last time, but no lot found!"
                    elif _last_received_as == "serial":
                        if not bool(inputs["serial"]):
                            exception_found = True
                            _res["msg"] = "The product code " + str(inputs["product_code"]) + " was received as serial-based last time, so serials required!"
                    else:
                        if bool(inputs["lot_number"]) or bool(inputs["serial"]):
                            exception_found = True
                            _res["msg"] = "The product code " + str(inputs["product_code"]) + " was received as no-tracking last time, so no lots or serials allowed!"
                    ...
                    if bool(inputs["serial"]):
                        ...
                        if bool(_found):
                            ...
                            if self.is_expiration_valid(None, inputs["expiry_date"]):
                                ...
                                _res["msg"] = "The serial " + str(inputs["serial"]) + " is valid but invalid expiration date(" + str(inputs["expiry_date"]) + ")!"
                        ...
                        if bool(_found):
                            if _found[-1].product_id[0].id != product_id:
                                _res["msg"] = "The lot " + str(inputs["lot_number"]) + " is not associated with the product code " + str(inputs["product_code"]) + "!"
                            else:
                                if self.is_expiration_valid(_found[-1].expiration_date, inputs["expiry_date"]):
                                    ...
                                    _res["msg"] = "The lot " + str(inputs["lot_number"]) + " is valid but it is already in use with an expiration date(" + str(_found[-1].expiration_date) + ")!"
                        else:
                            if self.is_expiration_valid(None, inputs["expiry_date"]):
                                _res["is_ok"] = True
                                _res["product_tracking"] = "lot"
                                ...
                                _res["msg"] = "The lot " + str(inputs["lot_number"]) + " is valid but invalid expiration date(" + str(inputs["expiry_date"]) + ")!"
                    else:
                        _res["is_ok"] = True
                        _res["msg"] = None
        else:
    ...
    def get_item_inbounds(self, _locations, _product, start_date, end_date, *, lot_serial=None):
        ...
        _tuple_list = list()
        query = """SELECT SML.company_id, SML.product_id, SM.origin, SM.warehouse_id, PO.partner_id, POL.product_qty AS ordered_qty, SML.qty_done AS received_qty, SML.location_dest_id AS location_id, SML.lot_id, SML.write_date, SML.write_uid FROM stock_move_line AS SML JOIN stock_move AS SM ON SML.move_id = SM.id JOIN purchase_order_line AS POL ON SM.purchase_line_id = POL.id JOIN purchase_order AS PO ON POL.order_id = PO.id WHERE SML.state = 'done' AND SML.company_id = %d AND SML.product_id = %d AND SML.location_dest_id IN (%s) AND SML.write_date BETWEEN '%s' AND '%s'"""
