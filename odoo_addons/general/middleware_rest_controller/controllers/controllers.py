...
class OdooRestController(http.Controller):
        ...
        if bool(res) and isinstance(res, str):
            ...
            if self.mode == "dev":
                ...
                for frame in tb.stack:
                    _obj = dict()
                    _obj["details"] = res.split("The issue related to: ")[1]
            ...
            self.response = {
                "status": "success",
                "message": _("Execution was successfully completed."),
                "data": "" if not bool(res) else res
            }
    ...
    def po_creation(self, **params):
        if self.is_authentic(params):
            try:
                ...
                self.response_handler("The issue related to: " + e.__str__())
    ...
    @http.route(api_ver + "get-purchase-order-location-n-vendor-addresses", type="json", auth="none", methods=["POST"], csrf=False)
    ...
    @http.route(api_ver + "get-sale-order-location-n-customer-addresses", type="json", auth="none", methods=["POST"], csrf=False)
    ...
    def list_suggestive_lots(self, **params):
        if self.is_authentic(params):
            try:
                inventories = Inventories(company_id=self.company_id, user_id=self.user_id)
