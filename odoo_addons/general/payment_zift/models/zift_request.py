...
class ZiftAPI():
    """Zift Gateway API integration.

    This class allows contacting the Zift API with simple operation
    requests. It implements a *very limited* subset of the complete API
    (http://developer.zift/api/reference); namely:
    ...
    def __init__(self, acquirer):
        ...
        if acquirer.state == 'enabled':
            self.url = 'https://sandbox-secure.zift.io/gates/xurl'
        else:
            self.url = 'https://sandbox-secure.zift.io/gates/xurl'

        self.state = acquirer.state
        self.name = acquirer.zift_login
        self.transaction_key = acquirer.zift_transaction_key

    def _zift_request(self, data):
        _logger.info('_zift_request: Sending values to URL %s, values:\n%s', self.url, data)
        resp = requests.post(self.url, json.dumps(data))
        resp.raise_for_status()
        resp = json.loads(resp.content)
        _logger.info("_zift_request: Received response:\n%s", resp)
    ...
    def create_customer_profile(self, partner, opaqueData):
        """Create a payment and customer profile in the Zift backend.

        Creates a customer profile for the partner/credit card combination and links
        a corresponding payment profile to it. Note that a single partner in the Odoo
        database can have multiple customer profiles in Zift (i.e. a customer
    ...
    def create_customer_profile_from_tx(self, partner, transaction_id):
        """Create an Zift payment/customer profile from an existing transaction.
        ...
        response = self._zift_request(values)
    ...
    def auth_and_capture(self, token, amount, reference):
        ...
        :param str reference: used as "invoiceNumber" in the Zift backend
        ...
        values = {
            'createTransactionRequest': {
                ...
                'transactionRequest': {
                    ...
                    'amount': str(amount),
                    'profile': {
                        'customerProfileId': token.zift_profile,
                        'paymentProfile': {
    ...
    def zift(self, token, amount, reference):
    ...
    def test_authenticate(self):
        """Test Zift communication with a simple credentials check.
