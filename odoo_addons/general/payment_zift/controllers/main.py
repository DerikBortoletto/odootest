...
class ZiftController(http.Controller):
    ...
    def zift_form_feedback(self, **post):
        """ Process the data returned by Zift after redirection.
        ...
        """
        _logger.info('Zift: entering form_feedback with post data %s', pprint.pformat(post))
        if post:
            request.env['payment.transaction'].sudo().form_feedback(post, 'zift')
        base_url = request.env['ir.config_parameter'].sudo().get_param('web.base.url')
        # Zift is expecting a response to the POST sent by their server.
        # This response is in the form of a URL that Zift will pass on to the
        # client's browser to redirect them to the desired location need javascript.
        return request.render('payment_zift.payment_zift_redirect', {
            'return_url': urls.url_join(base_url, "/payment/process")
        })

    @http.route(['/payment/zift/s2s/create_json_3ds'], type='json', auth='public', csrf=False)
    def authorize_s2s_create_json_3ds(self, verify_validity=False, **kwargs):
        ...
        res = {
            ...
            '3d_secure': False,
            'verified': True, #Zift does a transaction type of Authorization Only
                              #As Zift already verify this card, we do not verify this card again.
        }
        #token.validate() don't work with Zift.
        #Payments made via Zift are settled and allowed to be refunded only on the next day.
    ...
    @http.route(['/payment/zift/s2s/create'], type='http', auth='public', save_session=False)
    def zift_s2s_create(self, **post):
