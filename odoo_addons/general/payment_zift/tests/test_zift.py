...
class ZiftCommon(PaymentAcquirerCommon):

    @classmethod
    def setUpClass(cls, chart_template_ref=None):
        super().setUpClass(chart_template_ref=chart_template_ref)
        # zift only support USD in test environment
        cls.currency_usd = cls.env['res.currency'].search([('name', '=', 'USD')], limit=1)[0]
        # get the zift account
        cls.zift = cls.env.ref('payment.payment_acquirer_zift')
        cls.zift.write({
            'zift_login': 'dummy',
            'zift_transaction_key': 'dummy',
            'zift_signature_key': '00000000',
            'state': 'test',
        })
        # Be sure to be in 'capture' mode
        # self.zift.auto_confirm = 'confirm_so'


@odoo.tests.tagged('post_install', '-at_install', '-standard', 'external')
class ZiftForm(ZiftCommon):

    def test_10_Zift_form_render(self):
        self.assertEqual(self.zift.state, 'test', 'test without test environment')
        ...
        form_values = {
            'x_login': self.zift.zift_login,
            'x_amount': '56.16',
            'x_show_form': 'PAYMENT_FORM',
            'x_type': 'AUTH_CAPTURE',
            'x_method': 'CC',
            'x_fp_sequence': '%s%s' % (self.zift.id, int(time.time())),
            'x_version': '3.1',
        ...
        self.assertEqual(data_set[0].get('data-action-url'), ' https://sandbox-secure.zift.io/gates/xurl', 'Zift: wrong data-action-url POST url')
        for el in tree.iterfind('input'):
            ...
            self.assertEqual(
                ...
                'Zift: wrong value for input %s: received %s instead of %s' % (values[1], values[2], form_values[values[1]])
            )

    @mute_logger('odoo.addons.payment_zift.models.payment', 'ValidationError')
    def test_20_zift_form_management(self):
        # be sure not to do stupid thing
        self.assertEqual(self.zift.state, 'test', 'test without test environment')

        # typical data posted by zift after client has successfully paid
        zift_post_data = {
        ...
        with self.assertRaises(ValidationError):
            self.env['payment.transaction'].form_feedback(zift_post_data, 'zift')

        tx = self.env['payment.transaction'].create({
            'amount': 320.0,
            'acquirer_id': self.zift.id,
        ...
        self.env['payment.transaction'].form_feedback(zift_post_data, 'zift')
        # check state
        self.assertEqual(tx.state, 'done', 'Zift: validation did not put tx into done state')
        self.assertEqual(tx.acquirer_reference, zift_post_data.get('x_trans_id'), 'Zift: validation did not update tx payid')
        ...
        # simulate an error
        zift_post_data['x_response_code'] = u'3'
        self.env['payment.transaction'].form_feedback(zift_post_data, 'zift')
        # check state
        self.assertNotEqual(tx.state, 'done', 'zift: erroneous validation did put tx into done state')


@odoo.tests.tagged('post_install', '-at_install', '-standard')
class ZiftS2s(ZiftCommon):
    def test_30_zift_s2s(self):
        # be sure not to do stupid thing
        zift = self.zift
        self.assertEqual(zift.state, 'test', 'test without test environment')

        # add credential
        # FIXME: put this test in master-nightly on odoo/odoo + create sandbox account
        zift.write({
            'zift_transaction_key': '',
            'zift_login': '',
        })
        self.assertTrue(zift.zift_test_credentials, 'Zift: s2s authentication failed')
        ...
        transaction.zift_s2s_do_transaction()
        self.assertEqual(transaction.state, 'done',)

        # switch to 'zift only'
        # create zift only s2s transaction & capture it
        self.zift.capture_manually = True
        transaction = self.env['payment.transaction'].create({
            'amount': 500,
            'acquirer_id': zift.id,
            'type': 'server2server',
        ...
        # create zift only s2s transaction & void it
        self.zift.capture_manually = True
        transaction = self.env['payment.transaction'].create({
            'amount': 500,
            'acquirer_id': zift.id,
        ...
        ghost_payment_token.zift_profile = '99999999999'
        # create normal s2s transaction
        transaction = self.env['payment.transaction'].create({
            'amount': 500,
            'acquirer_id': zift.id,
