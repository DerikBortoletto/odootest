odoo.define('payment_zift.payment_form', function (require) {
"use strict";
...
PaymentForm.include({
     ...
     */
    _createZiftToken: function (ev, $checkedRadio, addPmEvent) {
        var self = this;
        if (ev.type === 'submit') {
        ...
        if (formData.acquirer_state === 'enabled') {
            AcceptJs = 'https://js.zift/v3/AcceptUI.js';
        } else {
            AcceptJs = 'https://jstest.zift/v3/AcceptUI.js';
        }
    ...
    payEvent: function (ev) {
        ...
        if ($checkedRadio.length === 1 && this.isNewPaymentRadio($checkedRadio) && $checkedRadio.data('provider') === 'authorize') {
            this._createZiftToken(ev, $checkedRadio);
        } else {
            this._super.apply(this, arguments);
        }
    },
    ...
    addPmEvent: function (ev) {
        ...
        if ($checkedRadio.length === 1 && this.isNewPaymentRadio($checkedRadio) && $checkedRadio.data('provider') === 'authorize') {
            this._createZiftToken(ev, $checkedRadio, true);
        } else {
            this._super.apply(this, arguments);
        }
    },
