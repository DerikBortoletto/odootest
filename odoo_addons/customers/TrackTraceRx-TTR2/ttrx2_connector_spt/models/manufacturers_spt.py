...
class manufacturers_spt(models.Model):
    ...
    def FromOdooToTTRx(self, connector, values={}):
        trading_partner_uuid = values.get('res_partner_id') and \
                self.env['res.partner'].search([('id','=',values['res_partner_id'])],limit=1).uuid or None
        default_address_uuid = values.get('address_partner_id') and \
                self.env['res.partner'].search([('id','=',values['address_partner_id'])],limit=1).uuid or None
        var = {
