...
class res_company(models.Model):
    ...
    ttrx_api_url = fields.Char('TTrx2 API Url', compute="_compute_api_url",
                               store=False)
    ttrx_api_tested = fields.Boolean('TTrx2 Tested', default=False)
    connector_id = fields.Many2one('connector.spt', 'Connector')

    wharehouse_ttr = fields.Many2one('stock.warehouse',
                                     string="Default Warehouse", domain=[
            ('reception_steps', '=', 'three_steps')])

    lot_number = fields.Selection([
        ('always_required', 'Always Required'),
        ('optional_to_add_inventory_required_to_sell',
         'Optional to add inventory, required to sell'),
    ...
    edi_source = fields.Selection([
        ('require_product_reception_verification',
         'Require product reception verification'),
        ('move_to_inventory_unless_lot_missing',
         'Move to inventory, unless lot is missing'),
        ('move_to_inventory_unless_lot_or_serial_missing',
         'Move to inventory, unless lot or serial is missing'),
    ...
    unknown_products = fields.Selection([
        ...
        ('add_product', 'Add the product to the product database'), ],
        string='Unknown Products')
    ...
    def _compute_api_url(self):
        ...
        for company in self:
            if company.api_environment == 'production':
                company.ttrx_api_url = configParameter.sudo().get_param(
                    "default_tracktracerx_endpoint_prod",
                    "https://api.tracktraceweb.com/2.0")
                # company.ttrx_api_url = TTRX_PROD_ENDPOINT
            else:
                company.ttrx_api_url = configParameter.sudo().get_param(
                    "default_tracktracerx_endpoint_test",
    ...
    @api.model
    def send_request_to_ttr(self, request_url, request_data=None, method='GET',
        ...
        if self.ttrx_api_tested:
            if not bool(self.ttrx_api_url) or not bool(request_url) or not bool(
                    self.api_key):
                # self.ttrx_api_tested = False
                return None
            ...
            try:
                ...
                req = requests.request(method=method, url=api_url,
            ...
            if 200 <= req.status_code <= 299:
                ...
                _logger.warning(f"Status da responsta {req.status_code} Error creating or updating from portal {response_text} ")
        ...
        if self.ttrx_api_url and self.api_key:
            try:
                ...
                request = urllib.request.Request(
                    url=self.ttrx_api_url + '/products',
    ...
    def import_export(self):
        ...
        try:
            form_view = self.env.ref(
                'tracktrace_odoo_connector_spt.import_export_form_view_spt')
