...
class ResConfigSettingsInherit(models.TransientModel):
    ...
    raise_exception_on_cron = fields.Boolean('Raise Exception on Cron', default=False)
    ...
    def get_values(self):
        ...
        res.update(
            raise_exception_on_cron=bool(params.get_param('ttrx2_connector_spt.raise_exception_on_cron', default=False))
        )
        return res

    def set_values(self):
        super(ResConfigSettingsInherit, self).set_values()
        self.env['ir.config_parameter'].sudo().set_param("ttrx2_connector_spt.raise_exception_on_cron", self.raise_exception_on_cron)
