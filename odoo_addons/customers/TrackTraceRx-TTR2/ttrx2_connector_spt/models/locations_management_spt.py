...
class locations_management_spt(models.Model):
    ...
    def FromOdooToTTRx(self, connector, values={}):
        var = super(locations_management_spt,self).FromOdooToTTRx(connector=connector,values=values)
        #Não
        ...
        import uuid
        parent_location_uuid = uuid.uuid1()
    ...
    def FromTTRxToOdoo(self, connector, values):
        ...
        warehouse_id = connector.wharehouse_id
        ...
        if not bool(location_id):
            location_type = 'main'
            location_id = warehouse_id.lot_stock_id.id or None
