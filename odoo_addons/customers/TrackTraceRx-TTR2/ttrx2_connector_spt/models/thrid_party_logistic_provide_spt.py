...
class thrid_party_logistic_provide_spt(models.Model):
    ...
    communication_format = fields.Selection([('EPCIS', 'EPCIS'),
                                             ...
                                             string='Outbound EPCIS Generator Type', default="EPCIS") 
    ...
    def FromOdooToTTRx(self, connector, values={}):
        ...
        if bool(values.get('product_category_ids')):
            for categ in values['product_category_ids']:
                tt_id = self.env['product.category'].browse(categ).tt_id or False
                if bool(tt_id):
                    products_categories.add(tt_id)
        else:
            for categ in self.product_category_ids:
                if bool(categ.tt_id):
                    products_categories.add(categ.tt_id)
        products = set()
        if bool(values.get('product_spt_ids')):
            for prod in values['product_spt_ids']:
                uuid = self.env['product.spt'].browse(prod).uuid or False
                if bool(uuid):
                    products.add(uuid)
        else:
            for prod in self.product_spt_ids:
                if bool(prod.uuid):
                    products.append(prod.uuid)
        locations = set()
        if bool(values.get('location_spt_ids')):
            for loca in values['location_spt_ids']:
                uuid = self.env['locations.management.spt'].browse(loca).uuid
                if bool(uuid):
                    locations.add(uuid)
        else:
            for loca in self.location_spt_ids:
                if bool(loca.uuid):
                    locations.add(loca.uuid)
