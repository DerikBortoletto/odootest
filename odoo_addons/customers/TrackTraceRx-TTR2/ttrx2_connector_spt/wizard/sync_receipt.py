...
class SyncReceiptSptWizard(models.TransientModel):
    _name = 'sync.receipts.spt.wizard'
    _description = 'Receipts Sync Wizard'
    ...
    transfer_id = fields.Many2one('stock.picking', string='Transfer', domain=[('uuid','!=',False),('state','!=','done'),('picking_type_code','=','incoming')],
    ...
    def action_process(self):
        for record in self:
            ...
            if record.from_type == 'single':
                if bool(record.transfer_id) and bool(record.transfer_id.uuid):
                    ...
                    raise UserError('Enter a valid Transfer Receipt!')
            else:    
                regs = self.env['connector.spt'].import_transfers_inbound()
                total = len(regs)
        ...
        return {  # Recarrega a view após a importação para mostrar resultados
                ...
                "res_model": "sync.receipts.spt.wizard",
                "views": [[False, "form"]],
                "name": "Receipts Sync Wizard",
                "target": "new",
                "res_id": self.id,
        }
